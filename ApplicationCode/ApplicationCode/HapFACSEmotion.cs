    ...
    class HapFACSEmotion
    {
        ...
        public string Happiness(string intensity, string lastIntensity)
        {
			...
			if( intensity == "E" || intensity == "D" )
				hyperText += hapFACS.AU6( "C", lastIntensity );
			else if( intensity == "C" || intensity == "B" )
				hyperText += hapFACS.AU6( "B", lastIntensity );
			else if (intensity == "A" || intensity == "0")
				hyperText += hapFACS.AU6( intensity, lastIntensity );
			else // Here, we scale the AU6 maximum to the B instead of E
				hyperText += hapFACS.AU6((Convert.ToDouble(intensity) / Constants.CIntensity).ToString(), lastIntensity);
			...
			if( intensity == "E" || intensity == "D")
				...
				hyperText += hapFACS.AU25( "A", lastIntensity );
			else if( intensity == "A" || intensity == "0" )
				hyperText += hapFACS.AU25( "0", lastIntensity );
			else // Here, we scale the AU25 maximum to the B instead of E
				hyperText += hapFACS.AU25( ( Convert.ToDouble( intensity ) / Constants.BIntensity ).ToString( ), lastIntensity );
